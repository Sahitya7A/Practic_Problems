/* Perform following operations on Hash Table using Linear Probing.
...
at index 2       student record: 402 sam 560
at index 3       student record: 502 tina 580
Press 1. Insert  2. Display     3. Search       4.Delete        5.Exit
3
Enter key of the student record to search
502
Student record is found at index 3
Student record: 502 tina 580
Press 1. Insert  2. Display     3. Search       4.Delete        5.Exit
4
...
void insert()
{
 ...
 //   if(h[index].rollno==0)
  //  {
       ...
       {
           index1=(index+i)%SIZE;
           if(h[index1].rollno==0)
           {
          // index1=(index+i)%SIZE;
           h[index1].rollno=no;
        strcpy(h[index1].name,name);
        h[index1].marks=marks;
        break;
       }
      /*  h[index].rollno=no;
        strcpy(h[index].name,name);
        h[index].marks=marks;
        //scanf("%d%s%d",&h[i].rollno,h[i].name,&h[i].marks);
    }*/
    if(index==SIZE)
    {
     /*  for(i=0;i<SIZE;i++)
